...
var canvas = canvasPreview.getContext("2d");
...
function drawText(text, x, y, fontFamily, fontSize, fontWeight, fontStyle, color) {
    canvas.textBaseline = top;
    canvas.font = fontStyle + " " + fontWeight + " " + fontSize + "px " + fontFamily;
...
function update() {
    ...
    for (var i = 0; i < idList.length; i++) {
        var id = idList[i];
        var select = document.getElementById("selectFontFamily" + id);
        drawText(
            document.getElementById("text" + id).value,
            document.getElementById("numberPosX" + id).value,
            document.getElementById("numberPosY" + id).value,
            "\"" + select.options[select.selectedIndex].textContent + "\"",
            document.getElementById("numberFontSize" + id).value,
            document.getElementById("rangeFontWeight" + id).value,
            document.getElementById("buttonItalic" + id).classList.contains("button-checked") ? "italic" : "",
